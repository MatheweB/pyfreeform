...
class TestWithinLine:
    def test_line_spans_rect(self):
        ...
        rect = cell.add_rect(fill="blue", width=0.5, height=0.5)
        line = cell.add_line(
            within=rect, start="top_left", end="bottom_right", color="red"
        )
    ...
    def test_line_left_to_right_within_rect(self):
        ...
        rect = cell.add_rect(fill="blue", width=0.5, height=0.5)
        line = cell.add_line(
            within=rect, start="left", end="right", color="red"
        )
...
class TestWithinCurve:
    def test_curve_within_rect(self):
        ...
        curve = cell.add_curve(
            within=rect, start="bottom_left", end="top_right",
            curvature=0.5, color="red"
        )
...
class TestWithinEllipse:
    def test_ellipse_centered_within_rect(self):
        ...
        ellipse = cell.add_ellipse(
            within=rect, at="center", rx=0.5, ry=0.5, fill="red"
        )
...
class TestWithinRect:
    def test_rect_within_rect(self):
        ...
        inner = cell.add_rect(
            within=outer, at="center", width=0.5, height=0.5, fill="red"
        )
...
class TestWithinPolygon:
    def test_polygon_triangle_within_rect(self):
        ...
        triangle = cell.add_polygon(
            [(0.5, 0.0), (1.0, 1.0), (0.0, 1.0)],
            within=rect, fill="red"
        )
...
class TestWithinText:
    def test_text_centered_within_rect(self):
        ...
        rect = cell.add_rect(fill="blue", width=0.5, height=0.5)
        text = cell.add_text(
            "A", within=rect, at="center", font_size=0.10, color="white"
        )
        # center of rect = (50, 50)
...
class TestCircularReference:
    def test_circular_reference_raises(self):
        ...
        dot_a._reference = dot_b
        dot_a._relative_at = (0.5, 0.5)
        dot_b._reference = dot_a
        dot_b._relative_at = (0.5, 0.5)
...
class TestModeSwitching:
    ...
    def test_move_to_clears_relative(self):
        scene, cell = _scene_with_cell(100)
        dot = cell.add_dot(at=(0.25, 0.75), color="red")
        assert dot.at is not None
        dot.move_to(10, 10)
