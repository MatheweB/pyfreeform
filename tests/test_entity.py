"""Tests for Entity base class methods, particularly fit_to_surface()."""
...
def test_fit_to_surface_dot():
    """Test fit_to_surface() with a Dot entity."""
...
def test_fit_to_surface_ellipse_rotated():
    """Test fit_to_surface() with a rotated Ellipse."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
...
def test_fit_to_surface_text():
    """Test fit_to_surface() with Text entity."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
...
def test_fit_to_surface_polygon():
    """Test fit_to_surface() with a Polygon entity."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
...
def test_fit_to_surface_no_cell():
    """Test that fit_to_surface() raises error when entity has no surface."""
    # Create standalone dot (not in a surface)
    dot = Dot(x=50, y=50, radius=10)

    with pytest.raises(ValueError, match="entity has no surface"):
        dot.fit_to_surface(0.8)


def test_fit_to_surface_invalid_scale_zero():
    """Test that fit_to_surface() rejects scale of 0.0."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
    dot = cell.add_dot(radius=0.25)

    with pytest.raises(ValueError, match="scale must be between"):
        dot.fit_to_surface(0.0)  # Too small


def test_fit_to_surface_invalid_scale_too_large():
    """Test that fit_to_surface() rejects scale > 1.0."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
    dot = cell.add_dot(radius=0.25)

    with pytest.raises(ValueError, match="scale must be between"):
        dot.fit_to_surface(1.5)  # Too large


def test_fit_to_surface_no_recenter():
    """Test fit_to_surface(recenter=False) maintains relative position."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
    ...
    dot.fit_to_surface(0.8, recenter=False)
...
def test_fit_to_surface_with_recenter():
    """Test fit_to_surface(recenter=True) centers entity in cell."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
    ...
    dot.fit_to_surface(0.8, recenter=True)
...
def test_fit_to_surface_method_chaining():
    """Test that fit_to_surface() supports method chaining."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]

    # Should support chaining
    ellipse = cell.add_ellipse(rx=2.5, ry=1.5).fit_to_surface(0.8)
...
def test_fit_to_surface_already_fits():
    """Test that fit_to_surface() scales up entities that are smaller than cell."""
    scene = Scene.with_grid(cols=2, rows=2, cell_size=20)
    cell = scene.grid[0][0]
...
def test_fit_to_surface_different_scales():
    """Test fit_to_surface() with different scale values."""
    ...
    dot1.fit_to_surface(1.0)  # 100% of cell
    dot2.fit_to_surface(0.5)  # 50% of cell
    dot3.fit_to_surface(0.1)  # 10% of cell
...
def test_fit_to_surface_brightness_based_scaling():
    ...
    ellipse1.fit_to_surface(scale1)
    ellipse2.fit_to_surface(scale2)
...
# =========================================================================
# Tests for fit_to_surface(at=) — position-aware mode
# =========================================================================


def test_fit_to_surface_at_center_same_as_default():
    ...
    dot_default.fit_to_surface(0.8)
    ...
    dot_at.fit_to_surface(0.8, at=(0.5, 0.5))
...
def test_fit_to_surface_at_rejects_corners_or_edges():
...
def test_fit_to_surface_at_accepts_center():
    ...
    assert dot.fit_to_surface(0.8, at="center") is not None


def test_fit_to_surface_at_rejects_edge_zero():
    ...
    with pytest.raises(ValueError, match="inside the bounds"):
        dot.fit_to_surface(0.8, at=(0.0, 0.5))


def test_fit_to_surface_at_rejects_edge_one():
    ...
    with pytest.raises(ValueError, match="inside the bounds"):
        dot.fit_to_surface(0.8, at=(0.5, 1.0))


def test_fit_to_surface_at_with_polygon():
    ...
    polygon.fit_to_surface(0.8, at=(0.75, 0.75))
...
def test_fit_to_surface_at_method_chaining():
    """fit_to_surface(at=) should support method chaining."""
    scene = Scene.with_grid(cols=1, rows=1, cell_size=100)
    cell = scene.grid[0][0]

    result = cell.add_dot(radius=2.0).fit_to_surface(0.8, at=(0.5, 0.5))
    assert result is not None


def test_fit_to_surface_backward_compat_no_at():
