...
def abstract_methods_anchors():
    ...
    # Polygon anchors
    poly = cells[3].add_polygon(shapes.hexagon(size=22), fill="#8b5cf6")
...
def entity_lifecycle():
    ...
    rect = cells[2].add_polygon(shapes.square(size=30), fill="#f59e0b")
    rect.rotate(45)
...
def lifecycle_transformation():
    ...
    # Original
    cells[0].add_polygon(shapes.square(size=40), fill="#3b82f6")
    cells[0].add_border(color="#e5e7eb", width=1)

    # After rotate
    rect2 = cells[1].add_polygon(shapes.square(size=40), fill="#3b82f6")
    rect2.rotate(45)
    cells[1].add_border(color="#e5e7eb", width=1)

    # After scale
    rect3 = cells[2].add_polygon(shapes.square(size=40), fill="#3b82f6")
    rect3.scale(0.6)
    cells[2].add_border(color="#e5e7eb", width=1)

    # Combined
    rect4 = cells[3].add_polygon(shapes.square(size=40), fill="#3b82f6")
    rect4.rotate(30)
    rect4.scale(0.8)
    cells[3].add_border(color="#e5e7eb", width=1)
...
def bounds_method_visual():
    ...
    cells[0].add_polygon(shapes.square(size=40), fill="none", stroke="#ef4444", stroke_width=1)
    cells[0].add_border(color="#e5e7eb", width=1)

    # Ellipse with bounds
    cells[1].add_ellipse(rx=25, ry=18, fill="#10b981")
    cells[1].add_polygon(shapes.square(size=50), fill="none", stroke="#ef4444", stroke_width=1)
    cells[1].add_border(color="#e5e7eb", width=1)

    # Polygon with bounds
    cells[2].add_polygon(shapes.star(points=5, size=22), fill="#f59e0b")
    cells[2].add_polygon(shapes.square(size=44), fill="none", stroke="#ef4444", stroke_width=1)
    cells[2].add_border(color="#e5e7eb", width=1)
...
def entity_inheritance_pattern():
    ...
    cells[4].add_polygon(shapes.triangle(size=30), fill="#3b82f6")
    cells[5].add_polygon(shapes.hexagon(size=15), fill="#8b5cf6")
    cells[6].add_polygon(shapes.star(points=5, size=15), fill="#ec4899")
