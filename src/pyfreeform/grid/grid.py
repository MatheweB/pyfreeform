...
class Grid:
    ...
    def border(self, thickness: int = 1) -> Iterator[Cell]:
        ...
        for cell in self:
            ...
            on_border = (
                r < thickness  # Top edge
                or r >= self._rows - thickness  # Bottom edge
                or c < thickness  # Left edge
                or c >= self._cols - thickness  # Right edge
            )
            if on_border:
                yield cell
    ...
    def merge(
        ...
        if not cells:
            raise ValueError(f"No cells in region start={start}, end={end}")
    ...
    def __repr__(self) -> str:
        return (
            f"Grid({self._cols}x{self._rows}, cell_size=({self._cell_width}, {self._cell_height}))"
        )
