...
def development_workflow():
    ...
    # 3. Changes
    cells[2].add_polygon(Polygon.triangle(size=0.7), fill="#f59e0b")
    cells[2].add_border(color="#f59e0b", width=2)

    # 4. Tests
    cells[3].add_polygon(Polygon.hexagon(size=0.7), fill="#8b5cf6")
    cells[3].add_border(color="#8b5cf6", width=2)

    # 5. Format
    cells[4].add_polygon(Polygon.star(points=5, size=0.7), fill="#ec4899")
...
def code_quality_tools():
    ...
    # MyPy - Type Checking
    cells[1].add_polygon(Polygon.hexagon(size=0.7), fill="#3b82f6", stroke="#3b82f6", stroke_width=2)
    cells[1].add_polygon(Polygon.hexagon(size=0.4), fill="#3b82f6")
    cells[1].add_border(color="#e5e7eb", width=1)

    # Pytest - Testing
    cells[2].add_polygon(Polygon.triangle(size=0.7), fill="#10b981", stroke="#10b981", stroke_width=2)
    cells[2].add_polygon(Polygon.triangle(size=0.4), fill="#10b981")
...
def contribution_areas():
    ...
    for area in areas:
        ...
        if area["shape"] == "dot":
            ...
            cell.add_polygon(Polygon.triangle(size=0.7), fill=area["color"])
        elif area["shape"] == "hexagon":
            cell.add_polygon(Polygon.hexagon(size=0.7), fill=area["color"])
        elif area["shape"] == "star":
            cell.add_polygon(Polygon.star(points=5, size=0.7), fill=area["color"])
        elif area["shape"] == "square":
            cell.add_polygon(Polygon.square(size=0.7), fill=area["color"])
...
def testing_pyramid():
    ...
    scene.grid[0, 1].add_polygon(Polygon.triangle(size=0.7), fill="#3b82f6")
    scene.grid[0, 1].add_border(color="#e5e7eb", width=1)

    # Middle row: 2 cells (component)
    scene.grid[1, 0].add_polygon(Polygon.square(size=0.7), fill="#10b981")
    scene.grid[1, 1].add_polygon(Polygon.square(size=0.7), fill="#10b981")
    scene.grid[1, 0].add_border(color="#e5e7eb", width=1)
    scene.grid[1, 1].add_border(color="#e5e7eb", width=1)

    # Bottom row: 3 cells (unit)
    for col in range(3):
        scene.grid[2, col].add_polygon(Polygon.square(size=0.7), fill="#f59e0b")
...
def code_style_guidelines():
    ...
    scene.grid[0, 0].add_polygon(Polygon.hexagon(size=0.7), fill="#10b981", stroke="#10b981", stroke_width=2)
    scene.grid[1, 0].add_polygon(Polygon.star(points=5, size=0.7), fill="#10b981", stroke="#10b981", stroke_width=2)
    scene.grid[2, 0].add_ellipse(rx=25, ry=25, fill="#10b981", stroke="#10b981", stroke_width=2)

    # Bad examples (right column) - less structured
    scene.grid[0, 1].add_polygon(Polygon.triangle(size=0.7), fill="#ef4444", stroke="#ef4444", stroke_width=1)
    scene.grid[1, 1].add_dot(radius=20, color="#ef4444")
    scene.grid[2, 1].add_polygon(Polygon.square(size=0.7), fill="#ef4444", stroke="#ef4444", stroke_width=1)
